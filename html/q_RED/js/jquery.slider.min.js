...
(function ( $ ) {
    ...
    $.fn.parallaxSlider = function( options ) {
		...
		function goTo(elScrollTo, elAtNow)
		{			
			if (window.paralaxSliderDisable == true) return;
			
			settings.onBefore(elScrollTo, elAtNow);
			...
			_this.delay(settings.speed_delay).animate({
				...
				settings.onAfter(elScrollTo, elAtNow);
			});
		}
		function getEl(ind)
		{
		...
		{
			$(settings.SEL_bulletR).click(function(){
				var num = transformNum('+');
				if (num === numNow) return;
				goTo(getEl(num), getEl(numNow));
				numNow = num;
		...
		if (settings.SEL_bulletL)
		{
			$(settings.SEL_bulletL).click(function(){
				var num = transformNum('-');
				if (num === numNow) return;
				goTo(getEl(num), getEl(numNow));
				numNow = num;
			...
			if (_mouseDown) {
				_mouseDown = false; 
				  ...
				  window.removeEventListener('mouseup', touchEnd, false);
					goTo(getEl(touchNum), getEl(numNow));
					numNow = touchNum;
					if (settings.SEL_paging)
					{
						$(settings.SEL_paging).children().removeClass(settings.selected).eq(numNow).addClass(settings.selected);
